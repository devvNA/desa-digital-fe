...
<template>
    <div class="flex flex-col gap-4">
        <div id="Header" class="flex items-center justify-between">
            <div class="flex flex-col gap-2">
                <div class="flex gap-1 items-center leading-5 text-desa-secondary">
                    ...
                    <span>/</span>
                    <p class="last-of-type:text-desa-dark-green last-of-type:font-semibold capitalize">ubah bantuan
                        sosial</p>
                </div>
        ...
        <div v-if="generalError" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-2xl relative"
            role="alert">
        ...
        <form id="myForm" class="capitalize" @submit.prevent="handleSubmit">
            <div class="shrink-0 rounded-3xl p-6 bg-white flex flex-col gap-6 h-fit">
                ...
                <section id="Thumbnail" class="flex items-center justify-between">
                    ...
                    <div class="flex-1 flex items-center justify-between">
                        <div
                            ...
                            <img :src="socialAssistance.thumbnail_url || fallbackImage" alt="image"
                                class="size-full object-cover" @error="handleImageError" />
                        </div>
                        <div class="relative">
                            <input ref="fileInput" type="file" name="thumbnail" accept="image/*"
                ...
                <div v-if="getFieldError('thumbnail')" class="flex justify-end">
                    <span class="w-full max-w-[480px] text-left text-desa-red text-xs">{{ getFieldError('thumbnail')
                    }}</span>
                </div>
                ...
                <section id="Nama-Bantuan-Sosial" class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col gap-3 flex-1 shrink-0">
                        <Input v-model="socialAssistance.name" type="text" placeholder="Tentukan nama bantuan sosial"
                ...
                <section id="Kategori" class="flex items-start justify-between">
                    <p class="font-medium leading-5 text-desa-secondary w-[calc(424/904*100%)] pt-4">Pilih Opsi Kategori
                    </p>
                    <div class="grid grid-cols-2 flex-1 gap-6 shrink-0">
                        <label v-for="category in categories" :key="category.value"
                            ...
                            <input v-model="socialAssistance.category" type="radio" name="category"
                                ...
                                <div class="flex size-6 shrink-0">
                                    <img :src="category.icon" class="size-6 flex group-has-[:checked]:hidden"
                                        alt="icon">
                                    <img :src="category.checkedIcon" class="size-6 hidden group-has-[:checked]:flex"
                                        alt="icon">
                                </div>
                        </label>
                    </div>
                ...
                <div v-if="getFieldError('category')" class="flex justify-end">
                    <span class="w-full max-w-[480px] text-left text-desa-red text-xs">{{ getFieldError('category')
                    }}</span>
                </div>
                ...
                <section id="Nama-Pemberi-Bantuan" class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col gap-3 flex-1 shrink-0">
                        <Input v-model="socialAssistance.provider" type="text"
                            placeholder="Ketik nama orang atau organisasi" :error-message="getFieldError('provider')"
                ...
                <section id="Deskripsi" class="flex items-start justify-between">
                    <p class="font-medium leading-5 text-desa-secondary w-[calc(424/904*100%)] pt-4">Deskripsi Bantuan
                        Sosial</p>
                    <div class="flex flex-col gap-3 flex-1 shrink-0">
                        <textarea v-model="socialAssistance.description" rows="6"
                            ...
                            :class="getFieldError('description') ? 'ring-red-500 focus:ring-red-500' : ''"></textarea>
                ...
                <section id="Ketersediaan" class="flex items-center justify-between">
                    ...
                    <div class="flex flex-1 gap-6 shrink-0">
                        ...
                        <label v-for="option in availabilityOptions" :key="String(option.value)"
                            class="group flex w-full items-center h-14 rounded-2xl p-4 ring-[1.5px] ring-desa-background gap-2 has-[:checked]:ring-desa-dark-green has-[:checked]:bg-desa-foreshadow transition-setup">
                            <input v-model="socialAssistance.is_available" type="radio" name="is_available"
                                :value="option.value"
                                class="flex size-[18px] shrink-0 accent-desa-secondary checked:accent-desa-dark-green transition-setup">
                                <span
                                    class="font-medium leading-5 text-desa-secondary w-full group-has-[:checked]:text-desa-dark-green transition-setup">
                                ...
                                <div class="flex size-6 shrink-0">
                                    ...
                                    <img :src="option.checkedIcon" class="size-6 hidden group-has-[:checked]:flex"
                                        alt="icon">
                                </div>
                        </label>
                    </div>
                ...
                <div v-if="getFieldError('is_available')" class="flex justify-end">
                    <span class="w-full max-w-[480px] text-left text-desa-red text-xs">{{ getFieldError('is_available')
                    }}</span>
                </div>
