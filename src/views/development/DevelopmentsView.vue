...
const { success: developmentApplicantSuccess } = storeToRefs(developmentApplicantStore);
...
<template>
    <div class="flex flex-col gap-4">
        ...
        <section id="List-Pembangunan-Desa" class="flex flex-col gap-[14px]">
            ...
            <div
                v-if="developmentApplicantSuccess"
                class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-2xl relative"
                role="alert"
            >
                <span class="block sm:inline">{{ developmentApplicantSuccess }}</span>
                <button
                    type="button"
                    class="absolute top-1/2 -translate-y-1/2 right-4"
                    @click="developmentApplicantSuccess = null"
                >
                    <img
                        src="@/assets/images/icons/close-circle-secondary-green.svg"
                        class="flex size-6 shrink-0"
            ...
            <div
                v-if="generalError"
                class="relative mb-4 rounded-2xl border border-red-400 bg-red-100 px-4 py-3 text-red-700"
                role="alert"
            >
            ...
            >
                ...
                >
                    <div
                        class="group-[.active]:bg-desa-dark-green group-[.active]:text-white rounded-full py-[18px] flex justify-center items-center text-center text-desa-dark-green font-medium leading-5 transition-all duration-300"
                        @click="handleStatusTabChange('my-applications')"
                    >
            ...
            >
                <div class="flex w-[370px] shrink-0 flex-col gap-3">
                    <label class="relative group peer w-full valid">
                        ...
                        />
                        <div
                            class="absolute left-4 top-1/2 flex size-6 shrink-0 -translate-y-1/2 transform"
                        >
                            <img
                                src="@/assets/images/icons/box-search-secondary-green.svg"
                                class="hidden size-6 group-has-[:placeholder-shown]:flex"
                                alt="search icon inactive"
                            />
                            <img
                                src="@/assets/images/icons/box-search-black.svg"
                                class="flex size-6 group-has-[:placeholder-shown]:hidden"
            ...
            <div
                v-if="!isInitialLoading && !hasDevelopments && hasActiveFilters"
                class="flex justify-center"
            >
                <button
                    type="button"
                    @click="resetFilters"
                    class="inline-flex items-center rounded-2xl bg-desa-black px-12 py-3 text-sm font-medium text-white"
                >
                    Reset pencarian
                </button>
            </div>
            ...
            <PaginationUI
                v-if="!isInitialLoading && hasDevelopments"
                :meta="meta"
