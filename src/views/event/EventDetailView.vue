...
const { createEventParticipant, fetchEventParticipants } = eventParticipantStore;
const { error: participantError, loading: participantLoading } = storeToRefs(eventParticipantStore);

const authStore = useAuthStore();
const { user } = storeToRefs(authStore);

const fallbackImage = new URL("@/assets/images/thumbnails/kk-event-desa-1.jpeg", import.meta.url)
    .href;
...
const paymentSuccessMessage = ref(
    "Pembayaran Berhasil! Pendaftaran event Anda telah dikonfirmasi.",
);
...
function syncParticipantPrice() {
    eventParticipant.value.total_price =
        eventParticipant.value.quantity * Number(data.value.price ?? 0);
}
...
const paymentErrorMessage = computed(
    () => paymentError.value ?? extractErrorMessage(participantError.value),
);
...
function createMidtransConfigError() {
    return new Error(
        "Konfigurasi pembayaran Midtrans belum lengkap. Periksa environment aplikasi.",
    );
}
            ...
            if (existingScript) {
                existingScript.addEventListener(
                    ...
                    () => {
                        ...
                        reject(
                            new Error(
                                "Midtrans Snap berhasil dimuat, tetapi tidak dapat digunakan.",
                            ),
                        );
                    },
                    { once: true },
                ...
                existingScript.addEventListener(
                    ...
                    () =>
                        reject(
                            new Error("Gagal memuat Midtrans Snap. Coba lagi dalam beberapa saat."),
                        ),
                    { once: true },
                );
...
const handleSubmitEventParticipant = async () => {
    ...
    snap.pay(snapToken, {
        ...
        onPending() {
            paymentError.value =
                "Pembayaran belum selesai. Pendaftaran event otomatis dibatalkan dan tidak masuk ke riwayat acara Anda.";
        },
        onError(result) {
            paymentError.value =
                result?.status_message ??
                "Pembayaran gagal. Pendaftaran event otomatis dibatalkan dan tidak masuk ke riwayat acara Anda.";
        },
        onClose() {
            paymentError.value =
                "Popup pembayaran ditutup sebelum transaksi selesai. Pendaftaran event otomatis dibatalkan dan tidak masuk ke riwayat acara Anda.";
        },
    });
};
...
<template>
    <div class="flex flex-col gap-4">
        <div id="Header" class="flex items-center justify-between">
            ...
            <router-link
                :to="{ name: 'edit-event', params: { id: data.id } }"
        ...
        <div class="flex gap-[14px]">
            ...
            >
                ...
                <div class="flex flex-col gap-[14px]">
                    <div
                        v-for="value in recentParticipants"
                        :key="value.id"
                    ...
                    >
                        ...
                        >
                            <img
                                :src="getProfilePicture(value.profile_picture)"
                                class="w-full h-full object-cover"
                                alt="icon"
            ...
            >
                <div class="flex flex-col h-fit gap-6 rounded-2xl bg-white py-6 flex-1">
                    <h2 class="font-medium text-sm leading-[17.5px] text-desa-secondary px-6">
                        Detail Pembayaran
                    </h2>
                    ...
                    >
                        {{ participantLoading ? "Purchase Ticket..." : "Purchase Ticket" }}
                    </button>
                </div>
            </form>
        </div>
