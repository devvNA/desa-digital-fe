...
import { fallbackProfilePicture, fallbackThumbnail, handleImageError, normalizeImageUrl } from '@/helpers/socialAssistance';
...
const normalizedProfile = computed(() => {
    ...
    const gallery = Array.isArray(currentProfile.profile_images) ? currentProfile.profile_images : [];

    return {
        label: currentProfile.label ?? staticProfile.label,
        thumbnail: normalizeImageUrl(currentProfile.thumbnail, fallbackThumbnail),
        ...
        built_at: currentProfile.built_at ? formatDateWithDay(currentProfile.built_at) : formatDateWithDay(staticProfile.built_at),
        profile_images: gallery.map((image) => ({
            image: normalizeImageUrl(image?.image, fallbackThumbnail),
        })),
    };
});

const modalImage = computed(() => {
    return normalizedProfile.value.profile_images[activeImageIndex.value]?.image ?? normalizedProfile.value.thumbnail;
});
...
<template>
        ...
        <div v-else class="flex gap-[14px]">
            <section id="Nama-Desa"
                ...
                <div class="flex items-center justify-between">
                    <p class="font-medium leading-5 text-desa-secondary">{{ normalizedProfile.label }}</p>
                ...
                <div id="Nama-Desa" class="flex flex-col gap-[6px]">
                    ...
                    <div class="flex items-center gap-0.5">
                        <img src="@/assets/images/icons/location-secondary-green.svg" class="flex size-6 shrink-0"
                            alt="icon">
                        <span class="font-medium text-sm text-desa-secondary">{{ normalizedProfile.address }}</span>
                    </div>
                </div>
                <div id="Gallery" class="flex flex-col gap-[14px]">
                    <div data-modal="Modal-Gallery" data-gallery="@/assets/images/thumbnails/desa-gallery-1.png"
                        ...
                        <img :src="normalizedProfile.thumbnail" class="w-full h-full object-cover" alt="thumbnail"
                    ...
                    <div v-if="galleryImages.length" class="grid grid-cols-3 gap-[14px] w-[492px]">
                        <div v-for="(image, index) in galleryImages" :key="index" class="relative">
                            <button class="relative" @click="selectedImage(index)">
                                ...
                                <div v-if="index === 2 && normalizedProfile.profile_images.length > 3"
                                    ...
                                    <p class="font-semibold text-xl leading-6">{{
                                        normalizedProfile.profile_images.length - 1 }}+</p>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <p class="font-medium text-sm leading-[28px] text-desa-secondary">Jl. Mawar No. 45, RT 02/RW 03,
                        Kelurahan Sukajaya, Kecamatan Sukarame, Kota Jakarta, Jawa Barat, 40286</p>
                </div>
            </section>
                ...
                <div class="flex flex-col gap-[14px]">
                    <div class="flex items-center gap-3 w-[302px] shrink-0">
                        <div class="flex size-[54px] rounded-full bg-desa-foreshadow overflow-hidden">
                            <img :src="normalizedProfile.headman_photo" class="w-full h-full object-cover" alt="icon"
                                @error="onHeadmanPhotoError">
                        </div>
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold text-lg leading-5 text-desa-black">{{ normalizedProfile.headman }}
                            </p>
                    ...
                    <div class="flex items-center gap-3 w-[302px] shrink-0">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold text-lg leading-5">{{ normalizedProfile.people }}</p>
                    ...
                    <div class="flex items-center gap-3 w-[302px] shrink-0">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold text-lg leading-5">{{ normalizedProfile.agricultural_area }}</p>
                    ...
                    <div class="flex items-center gap-3 w-[302px] shrink-0">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold text-lg leading-5">{{ normalizedProfile.total_area }}</p>
                    ...
                    <div class="flex items-center gap-3 w-[302px] shrink-0">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold text-lg leading-5">{{ normalizedProfile.built_at }}</p>
    ...
    <div id="Modal-Gallery" class="modal fixed inset-0 flex flex-col h-screen z-40 bg-[#080C1ACC]"
        ...
        <div class="flex flex-col items-center justify-center m-auto">
            <div id="Main-Image-Container" class="flex aspect-[805/492] w-full max-w-[805px] overflow-hidden mx-auto">
                <img id="Selected-Image" :src="modalImage" class="size-full object-contain object-center"
                    alt="thumbnail">
            </div>
            <button
        ...
        <div class="flex flex-wrap items-center w-full border border-white/10 gap-4 p-4">
            <button v-for="(image, index) in normalizedProfile.profile_images" :key="index"
