...
const galleryError = computed(() => {
    ...
    const entry = Object.entries(allErrors).find(([key]) =>
        /^(images|image)(\.\[)/i.test(key),
    )
    const value = entry?.[1]
...
async function fetchData() {
    ...
    profile.value = {
        ...
        agricultural_area: response.agricultural_area ?? '',
        total_area: response.total_area ?? '',
        images: Array.isArray(response.profile_images)
            ? response.profile_images.map((img) => ({
                image: null,
                imageUrl: normalizeImageUrl(img?.image, fallbackThumbnail),
            }))
            : [],
    }
...
async function handleSubmit() {
    ...
    const result = await profileStore.updateProfile(route.params.id, {
        ...profile.value,
...
<template>
    <div class="flex flex-col gap-4">
        <div id="Header" class="flex items-center justify-between">
            <div class="flex flex-col gap-2">
                <div class="flex gap-1 items-center leading-5 text-desa-secondary">
                    <router-link :to="{ name: 'profile' }" replace
                        class="last-of-type:text-desa-dark-green last-of-type:font-semibold capitalize hover:underline">Profile
                        Desa</router-link>
                    <span>/</span>
        ...
        <div v-if="fetching" class="rounded-3xl bg-white p-6">
            <p class="font-medium text-desa-secondary">Memuat data profile desa...</p>
        </div>

        <template v-else>
            <div v-if="generalError"
                class="relative rounded-2xl border border-red-400 bg-red-100 px-4 py-3 text-red-700" role="alert">
                <span class="block sm:inline">{{ generalError }}</span>
                <button type="button" class="absolute right-4 top-1/2 -translate-y-1/2" @click="clearErrors">
                ...
                <div class="shrink-0 rounded-3xl p-6 bg-white flex flex-col gap-6 h-fit">
                    <section id="Photos" class="flex justify-between">
                        ...
                        <div class="photo-input-container flex flex-col gap-6 flex-1">
                            <div class="photo-form group/parent flex items-center justify-between gap-6 rounded-2xl transition-all duration-300"
                                :class="{
                                    'border border-desa-red/30 bg-desa-red/5 p-3': getFieldError('thumbnail'),
                                }">
                                <div
                                    class="Photo-Preview flex justify-center w-[120px] h-[100px] rounded-2xl overflow-hidden bg-desa-foreshadow shrink-0">
                                    <img :src="profile.thumbnail_url || fallbackThumbnail" alt="thumbnail desa"
                                        class="Photo size-full object-cover" @error="handlePreviewError" />
                                </div>
                                <div class="relative">
                                    <input ref="thumbnailInput" type="file" name="thumbnail" accept="image/*"
                                    ...
                                    <div class="action flex gap-3">
                                        <button type="button"
                                            ...
                                            <img src="@/assets/images/icons/send-square-white.svg" alt="upload icon"
                            ...
                            <div v-if="getFieldError('thumbnail')" class="text-left text-desa-red text-xs">
                            ...
                            <div v-for="(image, index) in profile.images" :key="index"
                                ...
                                <div
                                    ...
                                    <img :src="image.imageUrl || fallbackThumbnail" alt="galeri desa"
                                        class="Photo size-full object-cover" @error="handlePreviewError" />
                                </div>
                                <div class="relative flex gap-3">
                                    <input :id="`gallery-image-${index}`" type="file" name="images[]" accept="image/*"
                                        class="absolute opacity-0 left-0 top-0 size-0 -z-10"
                                    ...
                                    <button type="button" @click="removeImage(index)"
                                        ...
                                        <img src="@/assets/images/icons/trash-white.svg" alt="hapus gambar"
                                            class="size-6 shrink-0" />
                                    </button>
                                </div>
                            </div>
                            <div v-if="getImageFieldError(0)" class="text-left text-desa-red text-xs -mt-2">
                    ...
                    <section id="Nama-Desa" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <Input v-model="profile.name" type="text" placeholder="Ketik nama desa"
                                :error-message="getFieldError('name')" :icon="IconBuildingSecondaryGreen"
                    ...
                    <section id="Lokasi" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <textarea v-model="profile.address" placeholder="Ketik alamat desa" rows="6"
                            ...
                            <span v-if="getFieldError('address')" class="text-left text-desa-red text-xs">{{
                    ...
                    <section id="Kepala-Desa" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <Input v-model="profile.headman" type="text" placeholder="Pilih Kepala Desa"
                                :error-message="getFieldError('headman')" :icon="IconUserSecondaryGreen"
                    ...
                    <section id="Luas-Pertanian" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <Input v-model="profile.agricultural_area" type="number"
                                placeholder="Masukan total luas pertanian"
                                :error-message="getFieldError('agricultural_area')" :icon="IconGridSecondaryGreen"
                    ...
                    <section id="Luas-Area" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <Input v-model="profile.total_area" type="number" placeholder="Masukan total luas area"
                                :error-message="getFieldError('total_area')" :icon="IconGridSecondaryGreen"
                                :filled-icon="IconGridBlack" />
                        </div>
                    </section>
                    <hr class="border-desa-background" />
                    ...
                    <section id="Jumlah Penduduk" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <Input v-model="profile.people" type="number" placeholder="Masukan total penduduk desa"
                                :error-message="getFieldError('people')" :icon="IconPeopleSecondaryGreen"
                    ...
                    <section id="Deskripsi" class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col gap-3 flex-1 shrink-0">
                            <textarea v-model="profile.about" placeholder="Jelaskan lebih detail tentang desa terkait"
                                rows="6"
                                class="appearance-none outline-none w-full rounded-2xl ring-[1.5px] ring-desa-background focus:ring-desa-black py-4 px-4 gap-2 font-medium placeholder:text-desa-secondary transition-all duration-300"></textarea>
                            <span v-if="getFieldError('about')" class="text-left text-desa-red text-xs">{{
