...
const { headOfFamily, loading: headOfFamilyLoading, error: headOfFamilyError } = storeToRefs(headOfFamilyStore)
const { deleteHeadOfFamily } = headOfFamilyStore

const familyMemberStore = useFamilyMemberStore()
const { familyMember, loading: familyMemberLoading, error: familyMemberError } = storeToRefs(familyMemberStore)
const { deleteFamilyMember } = familyMemberStore

const fallbackImage = new URL('@/assets/images/photos/kk-photo-1.png', import.meta.url).href
const showModalDelete = ref(false)
const isHeadOfFamily = computed(() => route.query.type === 'head-of-family')
const detail = computed(() => (isHeadOfFamily.value ? headOfFamily.value : familyMember.value))
const loading = computed(() => (isHeadOfFamily.value ? headOfFamilyLoading.value : familyMemberLoading.value))
const error = computed(() => (isHeadOfFamily.value ? headOfFamilyError.value : familyMemberError.value))
const generalError = computed(() => {
    ...
    if (error.value && typeof error.value === 'object') {
        return 'Terjadi kesalahan saat memuat atau menghapus data anggota keluarga.'
    }
...
const fetchData = async () => {
    ...
    await familyMemberStore.fetchFamilyMember(route.params.id)
}
...
async function handleDelete() {
    if (isHeadOfFamily.value) {
        const isDeleted = await deleteHeadOfFamily(route.params.id)
    ...
    const isDeleted = await deleteFamilyMember(route.params.id)
    ...
    showModalDelete.value = false
    await router.replace({ path: '/family-member' })
}
...
<template>
        ...
        <header class="flex items-center justify-between">
            <div class="flex flex-col gap-2">
                <p class="flex items-center gap-1"><span class="leading-5 text-desa-secondary">Anggota
                        ...
                        class="font-medium leading-5 text-desa-dark-green">{{ isHeadOfFamily ? 'Detail Kepala Keluarga'
                            :
                            'Manage Anggota Keluarga' }}</span></p>
                <h1 class="font-semibold text-2xl leading-[30px]">
                    {{ isHeadOfFamily ? 'Detail Kepala Keluarga' : 'Manage Anggota Keluarga' }}</h1>
            </div>
            <button v-if="!isHeadOfFamily" data-modal="Modal-Delete"
                class="px-6 py-4 rounded-2xl bg-desa-red flex items-center gap-[10px]" @click="showModalDelete = true">
                <p class="font-medium leading-5 text-white">Hapus Data</p>
                <img src="@/assets/images/icons/trash-white.svg" alt="icon" class="size-5 shrink-0" />
            </button>
        </header>
        ...
        <div class="flex gap-[14px]">
            <div class="shrink-0 rounded-3xl p-6 bg-white flex flex-col gap-6 h-fit w-[calc(520/1000*100%)]">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-1">
                        <p class="font-semibold text-lg leading-[22.5px]">{{ formatAge(detail?.age,
                            detail?.date_of_birth) }}</p>
    ...
    <ModalDelete :show="showModalDelete" :title="'Hapus Anggota Keluarga?'" :loading="loading"
